...
class Messages::Facebook::MessageBuilder < Messages::Messenger::MessageBuilder
  ...
  def perform
    ...
    DeskFlowExceptionTracker.new(e, account: @inbox.account).capture_exception
    true
  end
  ...
  def contact_params
    begin
      ...
      if e.message.include?('2018218')
        ...
        DeskFlowExceptionTracker.new(e, account: @inbox.account).capture_exception unless @outgoing_echo
      end
    rescue StandardError => e
      result = {}
      DeskFlowExceptionTracker.new(e, account: @inbox.account).capture_exception
