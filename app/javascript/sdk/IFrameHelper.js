...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'deskflows_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$deskflows.position}`;
    if (window.$deskflows.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$deskflows.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('deskflows_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `deskflows-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('deskflows-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('deskflows-widget:', ''));
  ...
  setupAudioListeners: () => {
    const { baseUrl = '' } = window.$deskflows;
  ...
  events: {
    loaded: message => {
      updateAuthCookie(message.config.authToken, window.$deskflows.baseDomain);
      window.$deskflows.hasLoaded = true;
      const campaignsSnoozedTill = Cookies.get('cw_snooze_campaigns_till');
      IFrameHelper.sendMessage('config-set', {
        locale: window.$deskflows.locale,
        position: window.$deskflows.position,
        hideMessageBubble: window.$deskflows.hideMessageBubble,
        showPopoutButton: window.$deskflows.showPopoutButton,
        widgetStyle: window.$deskflows.widgetStyle,
        darkMode: window.$deskflows.darkMode,
        showUnreadMessagesDialog: window.$deskflows.showUnreadMessagesDialog,
        campaignsSnoozedTill,
        welcomeTitle: window.$deskflows.welcomeTitle,
        welcomeDescription: window.$deskflows.welcomeDescription,
        availableMessage: window.$deskflows.availableMessage,
        unavailableMessage: window.$deskflows.unavailableMessage,
        enableFileUpload: window.$deskflows.enableFileUpload,
        enableEmojiPicker: window.$deskflows.enableEmojiPicker,
        enableEndConversation: window.$deskflows.enableEndConversation,
      });
      IFrameHelper.onLoad({
      ...
      if (window.$deskflows.user) {
        IFrameHelper.sendMessage('set-user', window.$deskflows.user);
      }
      ...
      if (!window.$deskflows.resetTriggered) {
        dispatchWindowEvent({ eventName: DESKFLOWS_READY });
      }
    },
    error: ({ errorType, data }) => {
      dispatchWindowEvent({ eventName: DESKFLOWS_ERROR, data: data });
    ...
    setBubbleLabel(message) {
      setBubbleText(window.$deskflows.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$deskflows.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$deskflows.baseDomain);
    },

    postback(data) {
      dispatchWindowEvent({
        eventName: DESKFLOWS_POSTBACK,
        data,
      });
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `deskflows_live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$deskflows.hideMessageBubble);
    onLocationChangeListener();

    let className = 'woot-widget-bubble';
    let closeBtnClassName = `woot-elements--${window.$deskflows.position} woot-widget-bubble woot--close woot--hide`;
