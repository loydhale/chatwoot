...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$deskflows.type)) {
...
export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} woot-elements--${window.$deskflows.position}`;
...
const handleBubbleToggle = newIsOpen => {
  ...
  if (newIsOpen) {
    dispatchWindowEvent({ eventName: DESKFLOWS_OPENED });
  } else {
    dispatchWindowEvent({ eventName: DESKFLOWS_CLOSED });
    chatBubble.focus();
  }
};
...
export const onBubbleClick = (props = {}) => {
  ...
  const { isOpen } = window.$deskflows;
  if (isOpen === toggleValue) return;

  const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
  window.$deskflows.isOpen = newIsOpen;
