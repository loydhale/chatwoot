...
const whatsappAppId = computed(() => window.deskflowsConfig.whatsappAppId);
const whatsappConfigurationId = computed(
  () => window.deskflowsConfig.whatsappConfigurationId
);
...
onMounted(async () => {
  try {
    ...
    await setupFacebookSdk(
      whatsappAppId.value,
      window.deskflowsConfig?.whatsappApiVersion
    );
  } catch (error) {
