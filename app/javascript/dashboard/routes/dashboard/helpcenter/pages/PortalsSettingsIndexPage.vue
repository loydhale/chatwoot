...
const fetchSSLStatus = () => {
  if (!isOnDeskFlowCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on DeskFlow cloud)
  if (portalObj?.custom_domain && isOnDeskFlowCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
