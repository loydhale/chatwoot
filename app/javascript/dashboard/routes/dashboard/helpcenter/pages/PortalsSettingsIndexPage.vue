...
const fetchSSLStatus = () => {
  if (!isOnDeskFlowsCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on DeskFlows cloud)
  if (portalObj?.custom_domain && isOnDeskFlowsCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
