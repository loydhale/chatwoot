...
describe('#mutations', () => {
  describe('#SET_AGENTS', () => {
    it('set agent records', () => {
      ...
      mutations[types.default.SET_AGENTS](state, [
        { id: 1, name: 'Agent1', email: 'agent1@deskflows.app' },
      ]);
      expect(state.records).toEqual([
        {
          id: 1,
          name: 'Agent1',
          email: 'agent1@deskflows.app',
        },
      ]);
    });
  });
  ...
  describe('#ADD_AGENT', () => {
    it('push newly created agent data to the store', () => {
      const state = {
        records: [{ id: 1, name: 'Agent1', email: 'agent1@deskflows.app' }],
      };
      mutations[types.default.ADD_AGENT](state, {
        id: 2,
        name: 'Agent2',
        email: 'agent2@deskflows.app',
      });
      expect(state.records).toEqual([
        { id: 1, name: 'Agent1', email: 'agent1@deskflows.app' },
        { id: 2, name: 'Agent2', email: 'agent2@deskflows.app' },
      ]);
    });
  });
  ...
  describe('#EDIT_AGENT', () => {
    it('update agent record', () => {
      ...
      expect(state.records).toEqual([
        { id: 1, name: 'Agent2', email: 'agent2@deskflows.app' },
      ]);
    });
  });
