  ...
  class ContactSyncService
    ...
    def map_deskflow_to_ghl(contact)
      ...
      if contact.custom_attributes.present?
        payload['tags'] = contact.custom_attributes['ghl_tags'] if contact.custom_attributes['ghl_tags'].present?
      end
    ...
    def extract_ghl_id_from_contact(contact)
      ...
      if contact.identifier&.start_with?('ghl:')
        return contact.identifier.delete_prefix('ghl:')
      end
    ...
    def update_contact_attributes(contact, ghl_contact)
      ...
      if mapped[:custom_attributes]
        mapped[:custom_attributes] = (contact.custom_attributes || {}).merge(mapped[:custom_attributes])
      end
      if mapped[:additional_attributes]
        mapped[:additional_attributes] = (contact.additional_attributes || {}).merge(mapped[:additional_attributes])
      end
    ...
    def ghl_api_request(method, path, body = nil, query = nil)
      ...
      unless response.is_a?(Net::HTTPSuccess)
        raise "GHL API #{method.upcase} #{path} failed (#{response.code}): #{response.body}"
      end
