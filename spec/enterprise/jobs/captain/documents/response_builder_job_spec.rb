...
RSpec.describe Hudley::Documents::ResponseBuilderJob, type: :job do
  let(:assistant) { create(:captain_assistant) }
  let(:document) { create(:captain_document, assistant: assistant) }
  let(:faq_generator) { instance_double(Hudley::Llm::FaqGeneratorService) }
  let(:faqs) do
    [
  ...
  before do
    allow(Hudley::Llm::FaqGeneratorService).to receive(:new)
  ...
  describe '#perform' do
    ...
    context 'with different locales' do
      ...
      let(:spanish_faq_generator) { instance_double(Hudley::Llm::FaqGeneratorService) }
      ...
      it 'passes the correct locale to FAQ generator' do
        ...
        expect(Hudley::Llm::FaqGeneratorService).to have_received(:new)
          .with(spanish_document.content, 'portuguese', account_id: spanish_document.account_id)
      end
    end
    ...
    context 'when processing a PDF document' do
      ...
      let(:paginated_service) { instance_double(Hudley::Llm::PaginatedFaqGeneratorService) }
      let(:pdf_faqs) do
        [{ 'question' => 'What is in the PDF?', 'answer' => 'Important content' }]
      end

      before do
        allow(Hudley::Llm::PaginatedFaqGeneratorService).to receive(:new)
      ...
      it 'uses paginated FAQ generator for PDFs' do
        expect(Hudley::Llm::PaginatedFaqGeneratorService).to receive(:new).with(pdf_document, anything)
