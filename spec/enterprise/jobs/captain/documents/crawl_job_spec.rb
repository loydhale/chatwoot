...
RSpec.describe Atlas::Documents::CrawlJob, type: :job do
  ...
  describe '#perform' do
    context 'when CAPTAIN_FIRECRAWL_API_KEY is configured' do
      let(:firecrawl_service) { instance_double(Atlas::Tools::FirecrawlService) }
      let(:account) { document.account }
      let(:token) { Digest::SHA256.hexdigest("-key#{document.assistant_id}#{document.account_id}") }

      before do
        allow(Atlas::Tools::FirecrawlService).to receive(:new).and_return(firecrawl_service)
    ...
    context 'when CAPTAIN_FIRECRAWL_API_KEY is not configured' do
      ...
      let(:simple_crawler) { instance_double(Atlas::Tools::SimplePageCrawlService) }

      before do
        allow(Atlas::Tools::SimplePageCrawlService)
          .to receive(:new)
      ...
      it 'enqueues SimplePageCrawlParserJob for each discovered link' do
        page_links.each do |link|
          expect(Atlas::Tools::SimplePageCrawlParserJob)
    ...
    context 'when document is a PDF' do
      ...
      it 'processes PDF using PdfProcessingService' do
        pdf_service = instance_double(Atlas::Llm::PdfProcessingService)
        expect(Atlas::Llm::PdfProcessingService).to receive(:new).with(pdf_document).and_return(pdf_service)
      ...
      it 'handles PDF processing errors' do
        allow(Atlas::Llm::PdfProcessingService).to receive(:new).and_raise(StandardError, 'Processing failed')
