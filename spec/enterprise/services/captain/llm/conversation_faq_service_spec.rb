...
RSpec.describe Hudley::Llm::ConversationFaqService do
  ...
  let(:embedding_service) { instance_double(Hudley::Llm::EmbeddingService) }
  ...
  describe 'language handling' do
    context 'when conversation has different language' do
      ...
      it 'uses account language for system prompt' do
        expect(Hudley::Llm::SystemPromptsService).to receive(:conversation_faq_generator)
          .with('french')
          .at_least(:once)
          .and_call_original
