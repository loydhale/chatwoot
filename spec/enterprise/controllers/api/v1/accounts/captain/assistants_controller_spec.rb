...
RSpec.describe 'Api::V1::Accounts::Atlas::Assistants', type: :request do
      ...
      it 'creates a new assistant' do
        expect do
          post "/api/v1/accounts/#{account.id}/captain/assistants",
               ...
               as: :json
        end.to change(Atlas::Assistant, :count).by(1)
        ...
        expect(json_response[:config][:product_name]).to eq('DeskFlow')
  ...
  describe 'POST /api/v1/accounts/{account.id}/captain/assistants/{id}/playground' do
    ...
    context 'when it is an agent' do
      it 'generates a response' do
        chat_service = instance_double(Atlas::Llm::AssistantChatService)
        allow(Atlas::Llm::AssistantChatService).to receive(:new).with(assistant: assistant).and_return(chat_service)
