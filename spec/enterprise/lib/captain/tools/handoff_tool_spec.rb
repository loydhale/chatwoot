...
RSpec.describe Hudley::Tools::HandoffTool, type: :model do
    ...
    context 'when conversation exists' do
      ...
      context 'when handoff fails' do
        before do
          ...
          exception_tracker = instance_double(DeskFlowsExceptionTracker)
          allow(DeskFlowsExceptionTracker).to receive(:new).and_return(exception_tracker)
        ...
        it 'captures exception' do
          exception_tracker = instance_double(DeskFlowsExceptionTracker)
          expect(DeskFlowsExceptionTracker).to receive(:new).with(instance_of(StandardError)).and_return(exception_tracker)
