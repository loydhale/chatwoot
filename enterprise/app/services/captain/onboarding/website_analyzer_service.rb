class Hudley::Onboarding::WebsiteAnalyzerService < Llm::BaseAiService
  ...
  def analyze
    ...
    Rails.logger.error "[Hudley Onboarding] Website analysis error: #{e.message}"
  ...
  def fetch_website_content
    crawler = Hudley::Tools::SimplePageCrawlService.new(@website_url)
    ...
    if page_title.blank? && meta_description.blank? && text_content.blank?
      Rails.logger.error "[Hudley Onboarding] Failed to fetch #{@website_url}: No content found"
      return false
    end
    ...
    Rails.logger.error "[Hudley Onboarding] Failed to fetch #{@website_url}: #{e.message}"
    false
  end
  ...
  def build_analysis_prompt
    <<~PROMPT
...

      {
        ...
        "suggested_assistant_name": "A friendly assistant name (e.g., 'Hudley Assistant', 'Support Genie', etc.)",
  ...
  def parse_llm_response(response_text)
    ...
    Rails.logger.error "[Hudley Onboarding] JSON parsing error: #{e.message}"
    Rails.logger.error "[Hudley Onboarding] Raw response: #{response_text}"
